...
open class PagingViewController:
    ...
    public init(options: PagingOptions = PagingOptions()) {
        ...
        pageViewController = EMPageViewController(navigationOrientation: .horizontal)
    ...
    open func setBadge(_ index: Int, _ count: Int) {
        pagingController.h_setCount(index: index, count: count)
//        h_itemsCountDic[index] = count
//        (collectionView.cellForItem(at: IndexPath(row: index, section: 0)) as? PagingTitleCell)?.setRedPoint(count)
    }
