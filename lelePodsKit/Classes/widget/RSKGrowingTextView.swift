...
@IBDesignable open class RSKGrowingTextView: RSKPlaceholderTextView {
    ...
    open var maxLenght: Int = Int.max
    
    open var disableDoubleNewLine: Bool = false
    open var disableNewLine: Bool = false
    ...
    @objc private func handleRSKGrowingTextViewTextDidChangeNotification(_ notification: Notification) {
        ...
        if disableNewLine && text.contains("\n") {
            content = try! NSRegularExpression(pattern: "\n", options: []).stringByReplacingMatches(in: content, options: [], range: NSMakeRange(0, content.count), withTemplate: "")
            text = content
        }else if disableDoubleNewLine && text.contains("\n\n") {
            content = try! NSRegularExpression(pattern: "\n{2,}", options: []).stringByReplacingMatches(in: content, options: [], range: NSMakeRange(0, content.count), withTemplate: "\n")
            text = content
        }

        if content.count > maxLenght {
            text = String(content.prefix(maxLenght))
        }
